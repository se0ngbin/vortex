...
CONFIGS="$TCU_CONFIGS" make -j$(nproc) >> "$LOG_FILE" 2>&1

# Build sgemm_tcu test binary
echo "  Building sgemm_tcu test binary..."
make -C tests/regression/sgemm_tcu clean >> "$LOG_FILE" 2>&1
CONFIGS="-DITYPE=int8 -DOTYPE=int32" make -C tests/regression/sgemm_tcu >> "$LOG_FILE" 2>&1
